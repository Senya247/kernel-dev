...
static int __init etx_class_init(void)
{
	...
	if (IS_ERR(etx_class)) return PTR_ERR(etx_class);
...
static int __init etx_driver_init(void)
{
	...
	for (i = 0; i < etx_nr_devs; i++){
		...
		device_create(etx_class, NULL, MKDEV(etx_dev_major, etx_dev_minor+i), NULL, "koolchardrv%d", i);
	}
	return 0;
	fail:
		return result;
